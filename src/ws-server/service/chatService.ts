...
export default function setUpChatService(chatNamespace: Namespace) {
  ...
  chatNamespace.on("connection", (socket) => {
    ...
    socket.on("new_message", (data: { roomName: string; message: string }) => {
      ...
      chatNamespace.to(data.roomName).emit("receive_message", { message: data.message, userId: socket.id });
    });
